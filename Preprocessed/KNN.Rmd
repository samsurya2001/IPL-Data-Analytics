...
Imputing constant text for NA values of classifying variable which cant be NA for KNN
...
KNN model fitting with K=3

```{r}
knn = knn(train = train_data, test = test_data, cl = train_data$result, k=3)
```
...
```{r}
cm <- table(test_data$result,knn)
cm
```
Getting all evaluation metrics for KNN
