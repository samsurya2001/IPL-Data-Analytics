...
AdaBoost model fitting with 50 iterations

```{r}
model = boosting(result~.,data=train_data,boos = TRUE,mfinal = 50)
```
...
```{r}
cm <- table(test_data$result,y_pred$class)
cm
```
Getting all evaluation metrics for AdaBoost
